...
def non_max_suppression_2(prediction, num_classes, conf_thres=0.5, nms_thres=0.4):
    """
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        for c in unique_labels:
            ...
            while detections_class.size(0):
                ...
                ious = bbox_iou_2(max_detections[-1], detections_class[1:])
                # Remove detections with IoU >= NMS threshold
                detections_class = detections_class[1:][ious < nms_thres]
                #detections_class = detections_class[1:][ious < nms_thres[int(c.data.tolist())]]
...
def bbox_iou_2(box1, box2, x1y1x2y2=True):
    """
...
def regresbox(prediction, nms_thres=0.3):
    box_shed = prediction[prediction[:, -1] == 0.0]
    box_head = prediction[prediction[:, -1] == 1.0]
    box_body = prediction[prediction[:, -1] == 2.0]

    return torch.cat((box_shed, box_head, box_body))

    '''for box_s in box_shed:
        ious = bbox_iou2(box_s.unsqueeze(0), box_head)
        box_head = box_head[ious < nms_thres]
    
    #for box_b in box_body:
    #    ious = bbox_iou2(box_b.unsqueeze(0), box_head)
    #    box_head = box_head[ious < conf_thres]

    index_body = []
    for box_b in box_body:
        ious_shed = bbox_iou2(box_b.unsqueeze(0), box_shed)
        ious_head = bbox_iou2(box_b.unsqueeze(0), box_head)
        if torch.sum(ious_shed > nms_thres) == 0 and torch.sum(ious_head > nms_thres) == 0:
            #box_shed = box_shed[ious < conf_thres]
            index_body.append(True)
        else:
            index_body.append(False)
    box_body = box_body[index_body]

    #output[image_i] = torch.cat((box_shed, box_head, box_body))'''
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False, method = 'merge'):
    """
    ...
    # Box constraints
    min_wh, max_wh = 20, 4096  # (pixels) minimum and maximum box width and height
    ...
    if method == 'my':
        return non_max_suppression_2(prediction, nc, conf_thres, iou_thres)
